...
#define S_RX 4                // Define software serial RX pin (for GPS)
#define S_TX 3                // Define software serial TX pin (for GPS)
#define MOTOR D8              // Define motor signal pin D8
#define BUZZER D7              // Define buzzer signal pin D7
#define MOTOR_SWITCH D9       // Define motor switch signal pin D9 (act as the Engine Key Switch)

TinyGPSPlus gps;    //Create TinyGPSPlus variable
SoftwareSerial SoftSerial(S_RX, S_TX);    // Configure SoftSerial library (GPS)
...
void setup(void){
  ...
  digitalWrite(MOTOR, LOW); //initialize motor value as LOW
  WiFiConnection();  
}
...
void physical(){ // the function for system interaction that involves physical component
...
void loop() { // bulk of your Arduino sketch is executed

  while (SoftSerial.available() > 0) {  // make sure the system will only work if the GPS are in good condition
